...
const Home: React.FC = () => {
  ...
  const [savingsStyle, setSavingsStyle] = useState({ position: 'sticky', top: '3.5rem' });
    ...
    const handleScroll = () => {
      ...
      if (savingsElement && formElement) {
        ...
        if (savingsRect.bottom > formRect.bottom) {
          setSavingsStyle({ position: 'absolute', top: `${formRect.bottom - savingsRect.height}px` });
        } else {
      ...
      >
        ...
        {!isMobile && (
          <Box className="Home-savings"
            ...
            ref={savingsRef}
            style={{ ...savingsStyle, position: savingsStyle.position as 'sticky' | 'absolute' }}
          >
